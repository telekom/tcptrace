...
static timespec tv_slice_interval;
    ...
    char *argv[])
{
    ...
    /* turn the slice interval into a timespec */
    tv_slice_interval.tv_sec = (int)slice_interval;
    tv_slice_interval.tv_nsec =
	1000000000 * (slice_interval - tv_slice_interval.tv_sec);

    /* tell them what's happening */
    printf("mod_slice: generating data in %.3fsec slices to file %s\n",
	   slice_interval, SLICE_FILENAME);
    if (ldebug)
	printf("Slice interval tv: %u.%09u\n",
	       (unsigned)tv_slice_interval.tv_sec,
	       (unsigned)tv_slice_interval.tv_nsec);
...
%s -------- -------- -------- -------- -------- --------\n",
	...
	Mfprintf(pmf, "%8lu", pnow->tv_sec); break;
      case tf_unix_long:
	Mfprintf(pmf, "%8lu.%09u", pnow->tv_sec, pnow->tv_nsec); break;
    }
