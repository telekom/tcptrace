...
static char *ucb_get_host_letter(void *pucb);
static char *ucb_get_tsg_plotfile(void *pucb);
static int ucb_get_cb_type();


static char *ucb_get_host_letter(void *pucb) {
  return ((struct ucb *) pucb)->host_letter;
}

static char *ucb_get_tsg_plotfile(void *pucb) {
  return ((struct ucb *) pucb)->tsg_plotfile;
}

static int ucb_get_cb_type() {
    return TCPTRACE_CB_TYPE_UCB;
}

static struct generic_cb_ops ucb_ops = {
    &ucb_get_host_letter,
    &ucb_get_tsg_plotfile,
    &ucb_get_cb_type
};
    ...
    struct udphdr *pudp)
{
    ...
    pup->a2b.tsg_plotter = pup->b2a.tsg_plotter = NO_PLOTTER;

    /* init time sequence graphs */ 

    if (graph_tsg && !pup->ignore_pair) {

	snprintf(title, sizeof(title), "UDP %s_==>_%s (time sequence graph)",
		 pup->a_endpoint, pup->b_endpoint);
	
        pup->a2b.tsg_plotter = new_plotter((struct generic_cb *) &pup->a2b, NULL, title,
	    graph_time_zero ? "relative time" : "time",
	    graph_seq_zero ? "sequence offset" : "sequence number",
	    PLOT_FILE_EXTENSION);

	snprintf(title, sizeof(title), "UDP %s_==>_%s (time sequence graph)",
		 pup->b_endpoint, pup->a_endpoint);

        pup->b2a.tsg_plotter = new_plotter((struct generic_cb *) &pup->b2a, NULL, title,
	    graph_time_zero ? "relative time" : "time",
	    graph_seq_zero ? "sequence offset" : "sequence number",
	    PLOT_FILE_EXTENSION);
	...
	if (graph_time_zero) {
	    ...
	    plotter_nothing(pup->a2b.tsg_plotter, current_time);
	    plotter_nothing(pup->b2a.tsg_plotter, current_time);
	}
    ...
    void *plast)
{
    ...
    */
	...
	plotter_perm_color(from_tsgpl, "white");
	plotter_darrow(from_tsgpl, current_time, thisdir->data_bytes);
	plotter_uarrow(from_tsgpl, current_time, thisdir->data_bytes + uh_ulen);
	plotter_line(from_tsgpl,
		     current_time, thisdir->data_bytes,
		     current_time, thisdir->data_bytes + uh_ulen);	
    }
	...
	plotter_perm_color(to_tsgpl, "green");
	plotter_line(to_tsgpl,
		     current_time, otherdir->data_bytes,
		     current_time, otherdir->data_bytes + uh_ulen);
	if (uh_ulen > UDP_MIN_REQUEST_BYTES) {
	  char buf[16];
	  sprintf(buf, "R%d", uh_ulen);
	  plotter_text(to_tsgpl, current_time, otherdir->data_bytes + uh_ulen, "a", buf);
	}
    }
