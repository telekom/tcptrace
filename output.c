    ...
    tcp_pair *ptp)
{
   ...
   sv_print_count = 1; /* The first field (conn_#) gets printed in trace.c */
    ...
    etime_secs = etime / 1000000000.0;
    etime_nsecs = 1000000000 * (etime/1000000000.0 - (double)etime_secs);
	...
	*/ 
       fprintf(stdout,"%ld.%ld %s %ld.%ld %s",
	       (long)ptp->first_time.tv_sec, (long)ptp->first_time.tv_nsec, sp,
	       (long)ptp->last_time.tv_sec, (long)ptp->last_time.tv_nsec, sp);
       sv_print_count += 2;      
    }
    else {
	...
	if (etime == 0.0) {
		...
		StatLineI("wavg owin","bytes", 
			  (u_llong)(pab->owin_wavg/((double)etime/1000000000)), 
		  	  (u_llong)(pba->owin_wavg/((double)etime/1000000000)));
   	} 
    }
    ...
    etime_data1 = elapsed(pab->first_data_time,
			  pab->last_data_time); /* in nsecs */
    etime_data2 = elapsed(pba->first_data_time,
			  pba->last_data_time); /* in nsecs */
    /* fix from Rob Austein */
    StatLineF("data xmit time","secs","%7.3f",
	      etime_data1 / 1000000000.0,
	      etime_data2 / 1000000000.0);
    StatLineP("idletime max","ms","%s",
	      ZERO_TIME(&pab->last_time)?"NA":
	      (snprintf(bufl,sizeof(bufl),"%8.1f",(double)pab->idle_max/1000000.0),bufl),
	      ZERO_TIME(&pba->last_time)?"NA":
	      (snprintf(bufr,sizeof(bufr),"%8.1f",(double)pba->idle_max/1000000.0),bufr));
    ...
    etime /= 1000000000.0;  /* convert to seconds */
    if (etime == 0.0)
    ...
    /* TMO */ 
    StatLineF("tp_min_1s", "kbps", "%8.1f", tpob_get_min(&pab->tpob1), tpob_get_min(&pba->tpob1));
    StatLineF("tp_min_2s", "kbps", "%8.1f", tpob_get_min(&pab->tpob2), tpob_get_min(&pba->tpob2));
    StatLineF("tp_min_5s", "kbps", "%8.1f", tpob_get_min(&pab->tpob5), tpob_get_min(&pba->tpob5));
    StatLineF("tp_min_10s", "kbps", "%8.1f", tpob_get_min(&pab->tpob10), tpob_get_min(&pba->tpob10));

    StatLineF("tp_max_1s", "kbps", "%8.1f", tpob_get_max(&pab->tpob1), tpob_get_max(&pba->tpob1));
    StatLineF("tp_max_2s", "kbps", "%8.1f", tpob_get_max(&pab->tpob2), tpob_get_max(&pba->tpob2));
    StatLineF("tp_max_5s", "kbps", "%8.1f", tpob_get_max(&pab->tpob5), tpob_get_max(&pba->tpob5));
    StatLineF("tp_max_10s", "kbps", "%8.1f", tpob_get_max(&pab->tpob10), tpob_get_max(&pba->tpob10));
	...
	StatLineF("RTT min","ms","%8.1f",
		  (double)pab->rtt_min/1000000.0,
		  (double)pba->rtt_min/1000000.0);
	StatLineF("RTT max","ms","%8.1f",
		  (double)pab->rtt_max/1000000.0,
		  (double)pba->rtt_max/1000000.0);
	StatLineF("RTT avg","ms","%8.1f",
		  Average(pab->rtt_sum, pab->rtt_count) / 1000000.0,
		  Average(pba->rtt_sum, pba->rtt_count) / 1000000.0);
	StatLineF("RTT stdev","ms","%8.1f",
		  Stdev(pab->rtt_sum, pab->rtt_sum2, pab->rtt_count) / 1000000.0,
		  Stdev(pba->rtt_sum, pba->rtt_sum2, pba->rtt_count) / 1000000.0);
        if(!(csv || tsv || (sv != NULL)))
	  fprintf(stdout,"\n");
	StatLineF("RTT from 3WHS","ms","%8.1f",
		  (double)pab->rtt_3WHS/1000000.0,
		  (double)pba->rtt_3WHS/1000000.0);
        if(!(csv || tsv || (sv != NULL)))
	  fprintf(stdout,"\n");
	StatLineI("RTT full_sz smpls","", 
		  pab->rtt_full_count, pba->rtt_full_count);
	StatLineF("RTT full_sz min","ms","%8.1f",
		  (double)pab->rtt_full_min/1000000.0,
		  (double)pba->rtt_full_min/1000000.0);
	...
	StatLineF("RTT full_sz avg","ms","%8.1f",
		  Average(pab->rtt_full_sum, pab->rtt_full_count) / 1000000.0,
		  Average(pba->rtt_full_sum, pba->rtt_full_count) / 1000000.0);
	StatLineF("RTT full_sz stdev","ms","%8.1f",
		  Stdev(pab->rtt_full_sum, pab->rtt_full_sum2, pab->rtt_full_count) / 1000000.0,
		  Stdev(pba->rtt_full_sum, pba->rtt_full_sum2, pba->rtt_full_count) / 1000000.0);
        if(!(csv || tsv || (sv != NULL)))
	  fprintf(stdout,"\n");
...
");
	    ...
	    StatLineF("RTT avg (last)","ms","%8.1f",
		      Average(pab->rtt_sum_last, pab->rtt_count_last) / 1000000.0,
		      Average(pba->rtt_sum_last, pba->rtt_count_last) / 1000000.0);
	    StatLineF("RTT sdv (last)","ms","%8.1f",
		      Stdev(pab->rtt_sum_last, pab->rtt_sum2_last, pab->rtt_count_last) / 1000000.0,
		      Stdev(pba->rtt_sum_last, pba->rtt_sum2_last, pba->rtt_count_last) / 1000000.0);
	...
	StatLineF("min retr time","ms","%8.1f",
		  (double)((double)pab->retr_min_tm/1000000.0),
		  (double)((double)pba->retr_min_tm/1000000.0));
	StatLineF("max retr time","ms","%8.1f",
		  (double)((double)pab->retr_max_tm/1000000.0),
		  (double)((double)pba->retr_max_tm/1000000.0));
	StatLineF("avg retr time","ms","%8.1f",
		  Average(pab->retr_tm_sum, pab->retr_tm_count) / 1000000.0,
		  Average(pba->retr_tm_sum, pba->retr_tm_count) / 1000000.0);
	StatLineF("sdv retr time","ms","%8.1f",
		  Stdev(pab->retr_tm_sum, pab->retr_tm_sum2,
			pab->retr_tm_count) / 1000000.0,
		  Stdev(pba->retr_tm_sum, pba->retr_tm_sum2,
			pba->retr_tm_count) / 1000000.0);
    }
    ...
    long double etime)
{
    ...
    etime_secs = etime / 1000000000.0;
    etime_nsecs = 1000000000 * (etime/1000000000.0 - (double)etime_secs);
    snprintf(buf,sizeof(buf),"%lu:%02lu:%02lu.%09lu",
	    etime_secs / (60 * 60),
	    etime_secs % (60 * 60) / 60,
	    (etime_secs % (60 * 60)) % 60,
	    etime_nsecs);
