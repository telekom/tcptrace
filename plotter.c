...
struct plotter_info {
  ...
  struct generic_cb *p2plast;		/* the TCB that this goes with (if any) */
  timespec zerotime;		/* first time stamp in this plot (see -z) */
  ...
  int init_ack_written;
  timespec ts_prev_ack;
  u_long prev_ack;
  u_long prev_bytes_acked;
  u_long prev_window;

  int init_seg_written;
  timespec ts_prev_seg;
  u_long prev_seq_end;
  u_long prev_seg_len;
...
static char *xp_timestamp(PLOTTER pl, struct timespec time);
static char *TSGPlotName(struct generic_cb *plast, PLOTTER, char *suffix);
...
/*
 * WH 2011-06-16: changed precision to nanoseconds. However for compatibility
 * usecs will be produced in the output to xplot
 */
    ...
    char *suffix)
{
    ...
    snprintf(filename,sizeof(filename),"%s2%s%s",
	     plast->ops->get_host_letter(plast),
	     plast->ptwin->ops->get_host_letter(plast->ptwin), suffix);
    return(filename);
}
    ...
    char *suffix)
{
    ...
    ppi->init_seg_written = FALSE;
    ppi->init_ack_written = FALSE;
...
plotter_done(void)
{
	...
	if (!ignore_non_comp ||
	    ((ppi->p2plast != NULL) &&
	     (ppi->p2plast->ops->get_cb_type(ppi->p2plast) == TCPTRACE_CB_TYPE_UCB ||
	      ConnComplete(ppi->p2plast->ptp)))) {
	    Mfprintf(f,"go\n");
	    Mfclose(f);
	} else {
	    fname = ppi->p2plast->ops->get_tsg_plotfile(ppi->p2plast);
	    if (debug)
    ...
    ...)	   
{
   ...
   if(plot_argc > 0)
     {
	t1 = va_arg(ap, struct timespec);
	x1 = va_arg(ap, u_long);
     }
   if(plot_argc > 2)
     {
	t2 = va_arg(ap, struct timespec);
	x2 = va_arg(ap, u_long);
     }
   va_end(ap);
    ...
    PLOTTER pl)
{
   ...
   if(ppi->axis_switched) {   
      ...
      /* graph coordinates... */
      /*  X coord is timespec unless graph_time_zero is true */
...
void plotter_draw_xplz_ack(PLOTTER pl, 
			   timespec ts_prev, 
			   timespec ts_now, 
			   u_long prev_ack, 
			   u_long ack,
			   u_long prev_window_end, 
			   u_long window_end, 
			   char *rtt_dongle_color,
			   int is_triple_dupack) {
  
  struct plotter_info *ppi;
  char fmt[2000];
  int window = window_end - ack;
  int bytes_acked;
  ...
  if (!ppi->init_ack_written) {

    sprintf(fmt, "init_ack %s %u %s", 
	    xp_timestamp(pl, ts_now),
	    ack,
	    ack_color);

    ppi->init_ack_written = TRUE;
    ppi->ts_prev_ack = ts_now;

    DoPlot(pl, fmt);    

    bytes_acked = 0;
  } else {
    bytes_acked = ack - prev_ack;
  }

  timespec diff = ts_now;
  tv_sub(&diff, ppi->ts_prev_ack);

  if (window == ppi->prev_window
      && bytes_acked == ppi->prev_bytes_acked) {

    sprintf(fmt, "a%s %s", 
	    is_triple_dupack ? "3" : "",
	    xp_timestamp(pl, diff));
  } else {

    sprintf(fmt, "ack%s %s %u %u", 
	    is_triple_dupack ? "3" : "",
	    xp_timestamp(pl, diff),
	    bytes_acked,
	    window);
  ...
  ppi->ts_prev_ack = ts_now;
  ppi->prev_ack = ack;
  ppi->prev_window = window;
  ppi->prev_bytes_acked = bytes_acked;
...
void plotter_draw_xplz_data(PLOTTER pl, 
			    timespec ts_now, 
			    u_long seq_start, 
			    u_long seq_end, 
			    char *color, 
  ...
  if (!ppi->init_seg_written) {

    sprintf(fmt, "init_seg %s %u %s", 
	    xp_timestamp(pl, ts_now),
	    seq_start,
	    data_color);

    ppi->init_seg_written = TRUE;
    ppi->ts_prev_seg = ts_now;
    ppi->prev_seq_end = seq_start;
    ppi->prev_seg_len = 0;
  ...
  timespec diff = ts_now;
  tv_sub(&diff, ppi->ts_prev_seg);

  if (ppi->prev_seq_end == seq_start 
      && ppi->prev_seg_len == seg_len 
      && color == NULL) {
    ...
    sprintf(fmt, "%sseg %s %d %u", 
	    is_push_segment ? "p" : "",
	    xp_timestamp(pl, diff),
	    seq_start - ppi->prev_seq_end,
	    seg_len);
  }

  ppi->ts_prev_seg = ts_now;
  ppi->prev_seq_end = seq_end;
  ppi->prev_seg_len = seg_len;
