...
static int msg_count_unsupported_gtp_version = 3;

int get_gtp_header_length(struct gtphdr *pgtp)
{
  int length = 8;

  int gtp_version = pgtp->flags >> 5;

  if (gtp_version != 1) { /* GTPv1 */
    if (msg_count_unsupported_gtp_version > 0) {
      fprintf(stderr, "*** error: get_gtp_header_length(): got header with unsupported GTP protocol version %d\n", gtp_version);
    }
    return -1;
  }
  ...
  if (pgtp->flags & 7) {
    ...
    length += 4; // skip N-PDU number, sequence number and next extension header type

    if (pgtp->flags & 4) { // Extension headers

      uint8_t *ext_buf = ((uint8_t *) pgtp);

      do {
	length += ext_buf[length] << 2;
      } while (ext_buf[length - 1] > 0);
    }
  }
...
int
gettcp(
       struct ip *pip,       /* poiner to IP header */
       struct ip **ppip_inner, /* if not NULL, store inner IP header here (e.g. 
			      * inner IP header encapsulated in GTP)*/
       struct tcphdr **pptcp,
       void **pplast)
{
  struct udphdr *pudp;
  uint ret_val;
  int gtp_header_length;
  struct ip *pip_inner;

  ret_val = findheader(IPPROTO_TCP, pip, (void **)pptcp, pplast);
  if (ret_val == 0)
    return 0;

  /* no TCP header, check for UDP/GTP */

  ret_val = findheader(IPPROTO_UDP, pip, (void **)&pudp, pplast);
  if (ret_val != 0)
    return -1;

  if (htons(pudp->uh_sport) != UDP_PORT_GTP_U &&
      htons(pudp->uh_dport) != UDP_PORT_GTP_U)
    return -1;

  gtp_header_length = get_gtp_header_length((void *) pudp + 8);
  if (gtp_header_length < 0)
    return -1;

  pip_inner = (void *) pudp + 8 + gtp_header_length;

  if (ppip_inner)
    *ppip_inner = pip_inner;

  ret_val = findheader(IPPROTO_TCP, pip_inner, (void **) pptcp, pplast);
  return ret_val;
}
