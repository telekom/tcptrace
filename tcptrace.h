...
#define TPOB_MAX_ACK_INCREASE 65535
...
#define ZERO_TIME(ptv)(((ptv)->tv_sec == 0) && ((ptv)->tv_nsec == 0))
...
extern timespec first_packet;
extern timespec last_packet;
...
typedef struct segment {
    ...
    timespec	time;		/* time the segment was sent */
    struct segment *next;
    struct segment *prev;
} segment;
...
#define TCPTRACE_CB_TYPE_TCB 1
#define TCPTRACE_CB_TYPE_UCB 2

struct generic_cb_ops {
    char* (*get_host_letter)(void *cb);
    char* (*get_tsg_plotfile)(void *cb);        
    int (*get_cb_type)(void *cb);
};
...
#define SEGS_TO_REMEMBER 8
    ...
    timespec     previous_owin_sample_time;
    ...
    timespec	thru_firsttime;	/* time of first packet this interval */
    ...
    timespec	thru_lasttime;	/* time of previous segment */
    ...
    timespec	first_data_time;
    timespec	last_data_time;
    ...
    timespec	last_time;	/* last packet SENT from this side */
    ...
    tmo_tp_observer tpob1;
    tmo_tp_observer tpob2;
    tmo_tp_observer tpob5;
    tmo_tp_observer tpob10; 
...
#define UDP_PORT_GTP_C 2153
#define UDP_MIN_REQUEST_BYTES 128
...
void plotter_vtick(PLOTTER, timespec, u_long);
void plotter_utick(PLOTTER, timespec, u_long);
void plotter_uarrow(PLOTTER, timespec, u_long);
void plotter_tick(PLOTTER, timespec, u_long, char);
void plotter_text(PLOTTER, timespec, u_long, char *, char  *);
void plotter_temp_color(PLOTTER, char *color);
void plotter_ltick(PLOTTER, timespec, u_long);
void plotter_rtick(PLOTTER, timespec, u_long);
void plotter_rarrow(PLOTTER, timespec, u_long);
void plotter_plus(PLOTTER, timespec, u_long);
void plotter_perm_color(PLOTTER, char *color);
void plotter_line(PLOTTER, timespec, u_long, timespec, u_long);
void plotter_larrow(PLOTTER, timespec, u_long);
void plotter_htick(PLOTTER, timespec, u_long);
void plotter_dtick(PLOTTER, timespec, u_long);
void plotter_dot(PLOTTER, timespec, u_long);
void plotter_done(void);
void plotter_dline(PLOTTER, timespec, u_long, timespec, u_long);
void plotter_diamond(PLOTTER, timespec, u_long);
void plotter_darrow(PLOTTER, timespec, u_long);
void plotter_box(PLOTTER, timespec, u_long);
void plotter_arrow(PLOTTER, timespec, u_long, char);
void plotter_nothing(PLOTTER, timespec);
void plotter_invisible(PLOTTER, timespec, u_long);
void plotter_switch_axis(PLOTTER, Bool);
void plotter_draw_xplz_ack(PLOTTER pl, timespec ts_prev, timespec ts_now, u_long prev_ack, u_long ack, u_long prev_window_end, u_long window, char *rtt_dongle_color, int is_triple_dupack);
void plotter_draw_xplz_data(PLOTTER pl, timespec ts_now, u_long seq_start, u_long seq_end, char *color, int is_push_segment);
...
long double elapsed(timespec, timespec);
long double elapsed_in_sec(timespec, timespec);
void tv_sub(struct timespec *plhs, struct timespec rhs);
void tv_add(struct timespec *plhs, struct timespec rhs);
int tv_cmp(struct timespec lhs, struct timespec rhs);
Bool tv_same(struct timespec lhs, struct timespec rhs);
char *elapsed2str(long double etime);
int ConnReset(tcp_pair *);
int ConnComplete(tcp_pair *);
u_int SynCount(tcp_pair *ptp);
u_int FinCount(tcp_pair *ptp);
char *ts2ascii(timespec *);
char *ts2ascii_date(timespec *);
...
PLOTTER new_plotter(struct generic_cb *plast, char *filename, char *title,
...
void extend_line(PLINE pline, timespec xval, int yval);
...
#define PHYS_NONE       0 /* no link layer present */
#define PHYS_ETHER	1
#define PHYS_FDDI       2
...
typedef int pread_f(struct timespec *, int *, int *, void **,
		   int *, struct ip **, void **);
...
#endif /* GROK_ERF */
#ifdef GROK_TINYPCAP
        pread_f *is_pcap_file(char *);
#endif /* GROK_TINYPCAP */
#ifdef GROK_NETMON
        pread_f *is_netmon_file(char *);
#endif /* GROK_TINYNETMON */
#ifdef GROK_TINYPCAPNG
        pread_f *is_pcapng_file(char *);
#endif /* GROK_TINYPCAPNG */
...
#ifndef ETHERTYPE_S_VLAN
#define ETHERTYPE_S_VLAN        0x88a8
#endif /* ETHERTYPE_S_VLAN */

/* ETHERTYPE_P_MAP: Multiplexing and Aggregation Protocol - Value of ethertype
 * in Linux cooked capture files on an Android 4.4 Samsung Galaxy S5+ */

#ifndef ETHERTYPE_P_MAP
#define ETHERTYPE_P_MAP 0xda1a
#define P_MAP_HDR_SIZE 4
#endif /* ETHERTYPE_P_MAP */
