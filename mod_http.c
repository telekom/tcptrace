...
struct get_info {
    timespec get_time;		/* when CLIENT sent GET */
    timespec send_time;		/* when SERVER sent CONTENT */
    timespec lastbyte_time;	/* when SERVER sent last byte of CONTENT */
    timespec ack_time;		/* when CLIENT acked CONTENT */
...
static struct http_info {
    timespec c_syn_time;		/* when CLIENT sent SYN */
    timespec s_syn_time;		/* when SERVER sent SYN */
    timespec c_fin_time;		/* when CLIENT sent FIN */
    timespec s_fin_time;		/* when SERVER sent FIN */
...
/* local routines */
static timespec WhenSent(struct time_stamp *phead, struct time_stamp *ptail,
			u_long position);
static timespec WhenAcked(struct time_stamp *phead, struct time_stamp *ptail,
			 u_long position);
...
static void AddTS(struct time_stamp *phead, struct time_stamp *ptail,
		  u_long position);
static double ts2d(timespec *pt);
...
ts2d(timespec *pt)
{
    double d;
    d = pt->tv_sec;
    d += (double)pt->tv_nsec/1000000000;
    return(d);
}
...
/* return the timespec for the record of the smallest position >= "position" */
static timespec
WhenAcked(
    struct time_stamp *phead,
    struct time_stamp *ptail,
...
/* return the timespec for the record of the largest position <= "position" */
static timespec
WhenSent(
    struct time_stamp *phead,
    struct time_stamp *ptail,
	...
	return;
	  ...
	  /* Locate content-length field, if any */
	   case (GetStateFindContentLength): {
		...
		while (pch <= plast && *pch != '\r' && *pch != '\n') {
		  pch++;
		}

		/* pget->host_name = strndup(host_name, pch - host_name); */
		/* FSCK! strndup() dumps core (corrupt heap). Must be some nasty bug somewhere ... */
		/* for some reason the code below works though */

		length = pch - host_name;
		if (length > 63) 
		  length = 63;
		pget->host_name = malloc(64);
		strncpy(pget->host_name, host_name, length);
		pget->host_name[length] = 0;
	      }
...
HttpDoPlot()
{
	...
	if (p==NO_PLOTTER) {
	    ...
	    p = ph->pclient->plotter = new_plotter((struct generic_cb *) &ptp->a2b,
	...
	for (pget = ph->gets_head; pget; pget = pget->next) {
	    ...
	    /*	    plotter_text(p, pget->get_time, y_axis, "l", pget->get_string);*/
	    {
	      /* WH */
	      char buf[4096];
	      strcpy(buf, pget->host_name);
	      strcat(buf, pget->get_string);
	      plotter_text(p, pget->get_time, y_axis, "l", buf);
	    }
