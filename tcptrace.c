...
Bool csv = FALSE;
Bool tsv = FALSE;
Bool use_xplz_format = FALSE;
Bool calc_peak_goodput = TRUE;
...
char *sv = NULL;
char *default_window_scale_str = NULL;
int default_window_scale = 0;
...
struct timespec wallclock_finished;


/* first and last packet timestamp */
timespec first_packet = {0,0};
timespec last_packet = {0,0};
...
static struct ext_bool_op {
    ...
    {"use_xplz_format", &use_xplz_format, TRUE,
     "use .xplz format for xplot output"},
    {"calc_peak_goodput", &calc_peak_goodput, TRUE,
     "calculate 1s, 2s, 5s, 10s peak throughputs based on TCP acks (otherwise: use TCP data segments)"},
...
static void VerifyDefaultWindowScale(char *varname, char *value);
...
static struct ext_var_op {
    ...
    {"default_window_scale", &default_window_scale_str, VerifyDefaultWindowScale,
     "Default window scale parameter to be assumed when no TCP SYNs are present in the trace"}
...
void gettimeofday_timespec(struct timespec *ts)
{
  struct timeval tv;
...
//	udptrace_init();
  ...
  //  if (run_continuously) {
    ...
    gettimeofday_timespec(&wallclock_start);
    ...
    gettimeofday_timespec(&wallclock_finished);
    ...
    fprintf(stdout, "%selapsed wallclock time: %s, %d pkts/sec analyzed\n",
	    comment,
	    elapsed2str(etime),
	    (int)((double)pnum/(etime/1000000000)));
...
That will likely confuse the program, so be careful!\n", filename);
	...
	/* another sanity check, only understand ETHERNET and RAW frames right now */

	if (phystype != PHYS_ETHER && phystype != PHYS_NONE) {
...
for other packet types, I just don't have a place to test them\n\n");
	...
	if (do_udp) {
	  /* changed semantics: the option "-u" now implies that we're
	     not interested in TCP at all */
	  continue;
	}
...
static void VerifyDefaultWindowScale(char *varname, char *value) {
  default_window_scale = VerifyPositive(varname, value);
}
