    ...
    )
{
	...
	etime = (int)(elapsed(last_time, current_time)/1000000000.0);
	...
	if (IsNewBurst(pmc, ptcb, ptcbc, tcp)) {
		...
		int last_good_rtt = ptcb->rtt_last / 1000000000.0;
#ifdef OLD
    ...
    tcp_pair *ptp)   /* This conversation */
{
    ...
    module_conninfo *pmc;
                                  /* Pointer to a timespec structure.  The
				   * timespec structure becomes the time of
    ...
    module_conninfo *pmc)
{
	...
	etime = (int)(elapsed(pmc_previous->first_time,
			      pmc->first_time)/1000000000.0); /* convert ns to ms */
    ...
    module_conninfo *pmc)
{
    ...
    etime = (int)(elapsed(pmc->first_time,
			  pmc->last_time)/1000000000.0); /* convert ns to ms */
    ...
    f_testinside p_tester)	/* functions to test "insideness" */
{
	...
	if (is_telnet_conn(pmc)) {
		...
		elapsed(pmc->first_time,
			pmc->last_time)/1000000.0); /* convert us to ms */
    ...
    dyn_counter *psizes)
{
    ...
    temp = (int)(elapsed(pmc->last_time, current_time)/1000000.0); /* us to ms */
    ...
    struct tcphdr *tcp)
{
    ...
    {
	u_long etime_nsecs = elapsed(ptcbc->last_data_time, current_time);
	u_long last_rtt_nsecs = ptcb->rtt_last;
	if ((last_rtt_nsecs != 0) && (etime_nsecs < last_rtt_nsecs)) {
	    if (graph_tsg) {
		char buf[100];
		snprintf(buf,sizeof(buf),"short (%ld < %ld)", etime_nsecs, last_rtt_nsecs);
    ...
    module_conninfo *pmc)
{
    ...
    for (LOOP_OVER_BOTH_TCBS(dir)) {
	timespec last_packet = pmc->tcb_cache[dir].ptcb->last_time;

	/* elapsed time from last packet (in MICROseconds) */
	if (elapsed(last_packet,current_time) < 10*NS_PER_SEC) {
