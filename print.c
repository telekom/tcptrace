...
/* 	decimal = (ptime->tv_usec + 50) / 100;*/  /* for 4 digits */
	decimal = ptime->tv_nsec;  /* for 9 digits */

	now[24] = '\00';	/* nuke the newline */
	snprintf(buf,sizeof(buf), "%s.%09d %s", now, decimal, &now[20]);
...
/* 	decimal = (ptime->tv_usec + 50) / 100;*/  /* for 4 digits */
	decimal = ptime->tv_nsec;  /* for 9 digits */
	snprintf(buf,sizeof(buf), "%s.%09d", now, decimal);
